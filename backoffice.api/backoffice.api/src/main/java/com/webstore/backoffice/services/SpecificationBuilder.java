...
public class SpecificationBuilder<T> {
    ...
    public Specification<T> buildSpecification(JsonNode schema, Map<String, Object> filterParams) {
    ...
    private Optional<Predicate> handleFormattedField(CriteriaBuilder cb, Path<?> path,
        ...
        switch (format) {
            case "date-time":
                return handleDateTimeField(cb, path, value);
            case "date-time-no-year":
                return handleDateTimeNoYearField(cb, path, value);
            default:
                return Optional.empty();
        }
    }
