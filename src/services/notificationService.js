...
class NotificationService {
    ...
    async getNotificationByUserId(data){
        ...
        const notifications = await data.dbConnection.query(
            `UPDATE emails SET status = 'sent' WHERE recipient_id = $1 AND type = 'Notification' AND status IN ('pending', 'sent')`,
            [data.session.user_id]
        );

        const allUserNotifications = await data.dbConnection.query(
            `SELECT * FROM emails WHERE recipient_id = $1 AND type = 'Notification'`,
            [data.session.user_id]
        );
