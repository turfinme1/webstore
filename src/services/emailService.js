...
class EmailService {
    ...
    async prepareTestEmailData(data){
        ...
        if (emailTemplate.placeholders.includes("{address}")) {
            emailData.address = `<a href="${data.context.settings.url}:${ENV.FRONTOFFICE_PORT}/reset-password?token=RANDOMLY_GENERATED_TOKEN">Reset Password</a>`;
            ...
            emailData.address = `<a href="${data.context.settings.url}:${ENV.FRONTOFFICE_PORT}/auth/verify-mail?token=RANDOMLY_GENERATED_TOKEN">Verify Email</a>`;
        }
