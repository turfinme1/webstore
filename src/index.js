  ...
  switch (req.url) {
    ...
    case "/settlements":
      ...
      try {
        ...
        const queryText =
          'SELECT nm.ekatte,nm.name as "naseleno",km.name as "kmetstvo", ob.name as "obshtina", obl.name as "oblast" FROM public.naseleno_mqsto nm LEFT JOIN public.kmetstvo km ON km.id = nm.kmetstvo_id LEFT JOIN public.obshtina ob ON ob.id = km.obshtina_id LEFT JOIN public.oblast obl ON obl.id = ob.oblast_id WHERE nm.name = $1';
