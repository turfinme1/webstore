...
const MESSAGE_ERROR_STATUS_MAPPING = {
    [MESSAGE_ERROR_TYPES.NETWORK]: {
        status: MESSAGE_STATUS.PENDING,
        message: 'Network connection failed.'
    },
    [MESSAGE_ERROR_TYPES.SMTP]: {
        status: MESSAGE_STATUS.PENDING,
        message: 'Email server error occurred. The message could not be delivered due to SMTP issues.'
    },
    [MESSAGE_ERROR_TYPES.AUTH]: {
        status: MESSAGE_STATUS.FAILED,
        message: 'Authentication failed. Invalid credentials or authorization error.',
    },
    [MESSAGE_ERROR_TYPES.RATE_LIMIT]: {
        status: MESSAGE_STATUS.PENDING,
        message: 'Rate limit exceeded. Too many requests sent in a short period.',
    },
    [MESSAGE_ERROR_TYPES.INVALID_RECIPIENT]: {
        status: MESSAGE_STATUS.FAILED,
        message: 'Invalid recipient address.',
    },
    [MESSAGE_ERROR_TYPES.INVALID_INPUT]: {
        status: MESSAGE_STATUS.FAILED,
        message: 'Invalid input data.',
    },
    [MESSAGE_ERROR_TYPES.SUBSCRIPTION_NOT_FOUND]: {
        status: MESSAGE_STATUS.UNSUBSCRIBED,
        message: 'Push subscription not found or has been revoked.',
    },
    [MESSAGE_ERROR_TYPES.EXTERNAL_SERVER_ERROR]: {
        status: MESSAGE_STATUS.PENDING,
        message: 'External server error. The third-party service encountered an error.',
    },
};
    ...
    try {
        ...
        await client.query(`
            WITH to_expire AS (
                ...
                FROM message_queue
                WHERE status = $1
                AND now() > email_processing_started_at
                    + ((notification_settings->>'TTL')::int * INTERVAL '1 second')
                ORDER BY email_processing_started_at
                LIMIT $2
            )
            UPDATE message_queue mq
            SET status = $3, error_message = 'Message expired due to TTL'
            FROM to_expire te
            WHERE mq.id = te.id;`,
            [MESSAGE_STATUS.SENT, BATCH_SIZE, MESSAGE_STATUS.EXPIRED]
        );
        await client.query('COMMIT');
...
async function handlePushMessage(message, client, logger, messageService, settings) {
    ...
    if(subscription.rows.length === 0) {
        await client.query(`
            ...
            WHERE id = $3`,
            [MESSAGE_STATUS.UNSUBSCRIBED, 'Push subscription not found or has been revoked.', message.id]
        );

        await logger.info({
            code: 'TIMERS.PROCESS_MESSAGE_QUEUE.00004.NO_SUBSCRIPTION',
            short_description: 'No push subscription found',
            long_description: 'No push subscription found for the given message',
        });
        return { id: message.id, success: false };
    }
...
async function classifyMessageErrorAndSetStatus(error, message) {
    ...
    if (error.code === 'ECONNECTION' || error.code === 'ETIMEDOUT' || error.code === "ESOCKET") {
        ...
        if ([421, 450, 451].includes(error.responseCode)) {
            shouldRetry = true;
            errorType = MESSAGE_ERROR_TYPES.RATE_LIMIT;
        } else if ([511, 535].includes(error.responseCode)) {
            errorType = MESSAGE_ERROR_TYPES.AUTH;
        } else if ([550, 553].includes(error.responseCode)) {
            errorType = MESSAGE_ERROR_TYPES.INVALID_RECIPIENT;
        } else {
            shouldRetry = true;
            errorType = MESSAGE_ERROR_TYPES.SMTP;
        }
