...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const schema = await fetch("/notificationSchema.json").then((res) => res.json());
  const querySchema = await fetch("/notificationQueryParamsSchema.json").then((res) => res.json());
  const apiEndpoint = "/crud/notifications";
