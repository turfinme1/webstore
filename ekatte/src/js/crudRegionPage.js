...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("create-btn").addEventListener("click", () => {
    ...
    const createFormElement = createForm(regionSchema, "create-form");
  ...
  document.getElementById("search-btn").addEventListener("click", () => {
    formContainer.innerHTML = ""; // Clear the container
    const searchFormElement = createSearchForm(regionSchema, "search-form");
...
async function handleEdit(item) {
  ...
  try {
    ...
    const response = await fetch(`/${regionSchema.routeName}?id=${item.id}`);
    if (!response.ok) {
    ...
    const updateFormElement = createForm(regionSchema, "update-form", true);
